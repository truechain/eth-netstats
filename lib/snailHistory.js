...
{
	var best = _.max(this._items, 'height');
	if( _.isUndefined(best) ) {
		best = {
			height: 0,
			block: {},
			forks: [block],
...
{
	var worst = _.min(this._items, 'height');
	if( _.isUndefined(worst) ) {
		worst = {
			height: 0,
			block: {},
			forks: [block],
...
History.prototype.getCharts = function()
{
	if(this._callback !== null)
	{
		...
		this._callback(null, {
			...
			fruits : _.pluck( chartHistory, 'fruits' ),
		});
	}
}
