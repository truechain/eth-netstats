...
Node.prototype.setGeo = function(ip)
{
	...
	console.log( "GeoIp for " + ip + " is ", this.geo );
}
...
Node.prototype.setBasicStats = function(stats, callback)
{
	if( !_.isUndefined(stats) )
	{
		if( !_.isEqual(stats, {
			...
			gasPrice: this.stats.gasPrice,
			uptime: this.stats.uptime,
			isCommitteeMember: this.stats.isCommitteeMember,
		...
		}) )
		{
			...
			this.stats.isCommitteeMember = stats.isCommitteeMember || false;
			this.stats.isLeader = stats.isLeader || false;
